...
describe('LinksAndCovers Attribute', function (): void {
    ...
    describe('isClassLevel', function (): void {
        it('returns false when method is specified')
            ->expect(fn () => (new LinksAndCovers('App\\Services\\UserService', 'create'))->isClassLevel())
            ->toBeFalse();

        it('returns true when method is null')
            ->expect(fn () => (new LinksAndCovers('App\\Services\\UserService'))->isClassLevel())
            ->toBeTrue();
    });

    describe('getMethodIdentifier', function (): void {
        it('returns formatted method identifier')
            ->expect(fn () => (new LinksAndCovers('App\\Services\\UserService', 'create'))->getMethodIdentifier())
            ->toBe('App\\Services\\UserService::create');

        it('returns class-only identifier when no method')
            ->expect(fn () => (new LinksAndCovers('App\\Services\\UserService'))->getMethodIdentifier())
